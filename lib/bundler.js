...
class Bundler extends EventEmitter {
    constructor( argv ) {
        ...
        // Set up default config - entry paths get added later
        this.config = {
            debug: false,
            paths: [
                './node_modules',
        ...
        // Sync check for config file
        if ( this.argv.config ) {
            // Default config is overwritten by file config, which is in turn
            // overwritten by specified parameters
            this.config = Object.assign(
                this.config,
                this.getConfig( this.argv.config ),
                {
            ...
            if ( !Object.keys( this.config ).length ) {
                this.error( 'Empty config for common bundler - possible error' )
            }
        }
    }

    /**
     * Attempts to grab configuration from file path
     */
    getConfig( filepath ) {
        try {
    ...
    bundle() {
        ...
        this.getSourceRoots( entryGlob )
            ...
            .then( files => {
                ...
                this.verbose( Object.assign( this.config, {
                    entries: this.entries
                }))

                // Create bundler
                return browserify( Object.assign( this.config, {
                    entries: this.entries
                }))
            ...
            .then( bundler => {
                ...
                this.writer.once( EVENTS.WRITE_FINISH, () => {
                    ...
                    this.emit( EVENTS.BUNDLE_FINISH )
                })
            })
            .catch( err => {
